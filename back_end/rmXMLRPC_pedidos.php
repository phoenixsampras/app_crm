...
function VerificarCliente($conex, $partner = '')
{
  ...
  // Si es cliente nuevo tiene el prefijo NEW y no es entero
  if (is_int($partner->id)) {
    $filtroCliente = array(
      array(
        array(
          'id',
    ...
    $uid = login($conex);
    $models = ripcord::client("$url/xmlrpc/2/object");
    $rmListaCliente = $models->execute_kw($db, $uid, $password, 'res.partner', 'search_read', $filtroCliente, array(
      'fields' => array(
        'id',
        'name',
      ) ,
  ...
  if ($rmListaCliente > 0) {
    ...
    // print_r($rmListaCliente);
    // echo $rmListaCliente[0][id];
		return $rmListaCliente[0][id];
	}
	else {
    // echo "Falso";
    // print_r($rmListaCliente);
    ...
    $rm_dias_semana_bruto = explode(',', $partner->rm_dias_semana);
    // print_r($rm_dias_semana_bruto);
    for ($j = 0; $j < count($partner->rm_dias_semana); $j++) {
      $rm_dias_semana[] = intval($partner->rm_dias_semana[$j]);
    }
...
function rmRegistrarPedidoMasivo($conex, $user_id = '')
{
	...
	for ($i = 0; $i < count($jsondata->pedidos); $i++) {
		...
		// print_r($pedido->customerObj);

		$rmCustomer = VerificarCliente($conex, $pedido->customerObj);
    // echo "rmCustomer:" . $rmCustomer;
    // print_r ($rmCustomer);
		// $rmUserId=intval($_REQUEST['rmUserId']);
